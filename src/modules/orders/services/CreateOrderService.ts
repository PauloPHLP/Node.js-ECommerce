...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    const cutomerExists = await this.customersRepository.findById(customer_id);
    ...
    if (checkInexistingProducts.length)
      throw new AppError(`Could not product ${checkInexistingProducts[0].id}`);
